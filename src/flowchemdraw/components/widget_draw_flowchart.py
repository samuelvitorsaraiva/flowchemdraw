...
class graphic_draw(FigureCanvas):
    ...
    def on_mouse_press(self, event):
        if event.inaxes:
            ...
            if self.draw_connections['actived'] and event.button == 1:
                ...
                for dev in self.main_widget.devices.keys():

                    if dev != self.component:

                        dx = abs(self.main_widget.devices[dev]['draw_class'].position[0] - event.xdata)
                        dy = abs(self.main_widget.devices[dev]['draw_class'].position[1] - event.ydata)
                        ...
                        if dx < MIN_DISTANCE_TO_ACTIVATED and dy < MIN_DISTANCE_TO_ACTIVATED:
                            ...
                            point = self.main_widget.devices[dev]['draw_class'].get_nearby_connection(pos=(event.xdata, event.ydata))
                            ...
                            line = self.axes.plot(self.draw_connections['line_data'][0], self.draw_connections['line_data'][1], alpha=1, color='k')
                            self.main_widget.connections[f'{self.component}_{dev}'] = {'line': line}
                            self.main_widget.add_new_item_Qtree('connections', f'{self.component}_{dev}')

                self.draw_connections['line_connection'][0].remove()
                self.draw_connections = {'actived': False, 'line_connection': None, 'line_data': [[], []],
                                         'vertical': False, 'inflexion': True}

                self.main_widget.devices[self.component]['draw_class'].square_activation_connect(False)
            ...
            self.main_widget.active_item('', active=False)
            ...
            for dev in self.main_widget.devices.keys():
                ...
                if dx < MIN_DISTANCE_TO_ACTIVATED and dy < MIN_DISTANCE_TO_ACTIVATED:
                    if event.button == 1 and not self.move_componets['actived']:
                        self.main_widget.devices[dev]['draw_class'].square_activation_draw(True)
                        self.move_componets['actived'] = True
                        self.main_widget.active_item(dev, active=True)
                        self.component = dev

                    elif event.button == 3:
                        self.main_widget.devices[dev]['draw_class'].square_activation_connect(True)
    ...
    def on_mouse_release(self, event):
        if event.inaxes:
            ...
            if self.move_componets['actived']:
                ...
                dx = abs(self.main_widget.devices[self.component]['draw_class'].position[0] - event.xdata)
                dy = abs(self.main_widget.devices[self.component]['draw_class'].position[1] - event.ydata)
                ...
                if dx > MIN_DISTANCE_TO_ACTIVATED or dy > MIN_DISTANCE_TO_ACTIVATED:
                    ...
                    for dev in self.main_widget.devices.keys():
                        ...
                        position_dev = self.main_widget.devices[dev]['draw_class'].position

                        if position_dev[0] == position[0] and position_dev[1] == position[1]:
                            self.main_widget.devices[self.component]['draw_class'].square_activation_draw(False)
                            self.move_componets['actived'] = False
                            self.component = None
                            self.draw()
                            return
            ...
            if self.move_componets['actived']:
                self.main_widget.devices[self.component]['draw_class'].set_position(new_pos=position)
                self.main_widget.devices[self.component]['draw_class'].square_activation_draw(False)
                self.move_componets['actived'] = False
                self.component = None
                self.draw()
            ...
            if self.draw_connections['actived']:
                ...
                if self.draw_connections['inflexion']:
                    if not self.draw_connections['line_data'][0]:
                        ...
                        first_point = self.main_widget.devices[self.component]['draw_class'].get_nearby_connection(pos=(event.xdata, event.ydata))
